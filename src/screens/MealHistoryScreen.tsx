...
const MealHistoryScreen = () => {
  ...
  useEffect(() => {
    ...
    const fetchMeals = async () => {
      ...
      const meals: MealEntry[] = snapshot.docs.map((doc) => ({
        id: doc.id,
        name: doc.data().name,
        type: doc.data().type,
...
const styles = StyleSheet.create({
  ...
  card: { marginBottom: 10, borderRadius: 10, overflow: 'hidden' },
  cardContent: { backgroundColor: '#f9f9f9', borderRadius: 10, padding: 10 },
  mealRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  mealText: { fontSize: 16, color: '#000', flexShrink: 1 },
  timestamp: { marginTop: 5, fontStyle: 'italic', color: '#333' },
